...
-r              generate [r]esolver file (use this only if all corpora have resolver_vis_map.annis)
...
#check if resolver_vis_map.annis should be generated
...
{ 
	open(FLH,"$corpora{$i}/corpus.annis");
	@array = <FLH>;
	close(FLH);
	if ($i==1)
	{
		open OUTFILE, ">corpus.annis" or die $!;
	}
	else
	{
		open OUTFILE, ">>corpus.annis" or die $!;
	...
	foreach $line (@array)
	{
		...
		print OUTFILE "$corp_id\t$corp_name\t$corp_type\t$corp_version\t$corp_pre\t$corp_post\tFALSE\n"; 

	}


			#corpus_annotation.annis
			open(CORPANNO,"$corpora{$i}/corpus_annotation.annis");
			@array2 = <CORPANNO>;
			close(CORPANNO);
			if ($i==1)
			{
				open OUTFILE2, ">corpus_annotation.annis" or die $!;
			}
			else
			{
				open OUTFILE2, ">>corpus_annotation.annis" or die $!;
			...
			if ($i==1)
			{
				open OUTFILE4, ">node.annis" or die $!;
			}
			else
			{
				open OUTFILE4, ">>node.annis" or die $!;
			}
			open(NODEFILE,"$corpora{$i}/node.annis");
			@array4 = <NODEFILE>;
			close(NODEFILE);
			...
			#Create node_annotation.annis
			if ($i==1)
			{
				open OUTFILE4, ">node_annotation.annis" or die $!;
			}
			else
			{
				open OUTFILE4, ">>node_annotation.annis" or die $!;
			}
			open(NODEANNOFILE,"$corpora{$i}/node_annotation.annis");
			@array4 = <NODEANNOFILE>;
			close(NODEANNOFILE);
			...
			if ($i==1)
			{
				open OUTFILE4, ">text.annis" or die $!;
			}
			else
			{
				open OUTFILE4, ">>text.annis" or die $!;
			}
			open(TEXTFILE,"$corpora{$i}/text.annis");
			@array4 = <TEXTFILE>;
			close(TEXTFILE);
...
print OUTFILE "$max_corp_id\t$toplevelcorpus\tCORPUS\tNULL\t0\t$max_corp_post\tTRUE\n" or die "Error: Specified directory does not exist ";
close OUTFILE;

#Create component.annis
$max_comp_id = 0;
for ($i=1 ; $i <= $corp_count; $i++)
{ 
	if ($i==1)
	{
		open OUTFILE, ">component.annis" or die $!;
	}
	else
	{
		open OUTFILE, ">>component.annis" or die $!;
	}
	open(FLH,"$corpora{$i}/component.annis");
	@array = <FLH>;
	close(FLH);
...
{ 
	if ($i==1)
	{
		open OUTFILE, ">rank.annis" or die $!;
	}
	else
	{
		open OUTFILE, ">>rank.annis" or die $!;
	}
	open(FLH,"$corpora{$i}/rank.annis");
	@array = <FLH>;
	close(FLH);
	...
	foreach $line (@array)
	{
			...
			print OUTFILE "$uid\t$rank_pre\t$rank_post\t$rank_node_id\t$rank_comp\t$rank_parent\t$col7\n";

		   }
	}
	
		#Create edge_annotation.annis
			if ($i==1)
			{
				open OUTFILE3, ">edge_annotation.annis" or die $!;
			}
			else
			{
				open OUTFILE3, ">>edge_annotation.annis" or die $!;
			}
			open(FLH,"$corpora{$i}/edge_annotation.annis");
			@array3 = <FLH>;
			close(FLH);
...
#Create resolver_vis_map.annis if desired	
if($resolver == 1)
{
for ($i=1 ; $i <= $corp_count; $i++)
{ 

	open(FLH,"$corpora{$i}/resolver_vis_map.annis");
	@array = <FLH>;
	close(FLH);
...
open OUTFILE, ">resolver_vis_map.annis" or die $!;
